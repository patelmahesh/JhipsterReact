...
import com.panthera.IntegrationTest;
import com.panthera.config.EmbeddedKafka;
import com.panthera.config.KafkaSseConsumer;
import com.panthera.config.KafkaSseProducer;
...
class ReactKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/react-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/react-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/react-kafka/unregister"));
                return;
            }
        }
